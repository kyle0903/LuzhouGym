...
class LinepayService {
  constructor() {
    ...
    this.returnHost = config.frontendUrl;
  }
  ...
  createSignature(body, uri) {
    ...
    const message = `${this.channelSecret}/${
      this.version
  ...
  async requestPayment(orderData) {
    try {
      ...
      throw new Error(
        `LINE Pay request failed: ${response.data.returnMessage}`
      );
    } catch (error) {
  ...
  async confirmPayment(transactionId, amount) {
    try {
      ...
      throw new Error(
        `LINE Pay confirm failed: ${response.data.returnMessage}`
      );
    } catch (error) {
